...
- **카드 이름과 일부 스펠 카드 설명은 아직 원문으로 나옵니다.** 카드 DB(엔트리 1190)의
  카드명은 이미 한글로 바꿨지만, 게임은 **0x0d/0x8d 엔트리에 있는 다른 사본**을 읽습니다.
  이 타입은 게임 `code.bin` 기반 디코더가 있어야 풀립니다
  ([`docs/FORMAT.md` §10.2](docs/FORMAT.md)).
